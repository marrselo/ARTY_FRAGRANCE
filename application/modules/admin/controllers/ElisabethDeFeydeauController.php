...
class Admin_ElisabethDeFeydeauController extends ZExtraLib_Controller_Action {
    ...
    $fc = Zend_Controller_Front::getInstance();
    $confUpload = $fc->getParam('bootstrap')->getOption('upload');
    $this->view->rutaImagenObra = 'imgOuverages'.'/'.$datosObra['imgObra'];
    $formulario->insertId('idObra', $datosObra['idObra']);
    if ($this->_request->isPost()) {
    if($formulario->isValid($this->_params)){
        $this->cleanCache();
    ...
    public function newRealisationAction() {
        ...
        if ($this->_request->isPost()) {
            if ($formulario->isValid($this->_params)) {
                ...
                $this->cleanCache();
                $this->_redirect('/admin/elisabeth-de-feydeau/realisations/');
            }
        } else {
        ...
        if ($this->_request->isPost()) {
            ...
            if ($formulario->isValid($this->_params)) {
                ...
                if (!is_array($formulario->imagen->getFileName())) {
                    ...
                    $modelRealisations->editRealisationsIdioma($paramsIdioma,$this->_params['id'], $this->sessionAdmin->idiomaDetaful['idIdioma'],1);
                $this->cleanCache();
                $this->_redirect('/admin/elisabeth-de-feydeau/realisations/');
            }
        } else {
